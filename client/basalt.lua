...
nil end,setProjectDir=function(dab)cd=dab end,forceRenderUpdate=function()_da()end,debug=function(...)local dab={...}if
(_aa==nil)then print(...)return end;if(_aa.name~="basaltDebuggingFrame")then
if(_aa~=
daa.debugFrame)then daa.debugLabel:setParent(_aa)end end;local _bb=""for abb,bbb in pairs(dab)do
_bb=_bb..
tostring(bbb).. (#dab~=abb and", "or"")end
daa.debugLabel:setText("[Debug] ".._bb)for abb,bbb in pairs(bc(_bb,daa.debugList:getWidth()))do
daa.debugList:addItem(bbb)end
if(
daa.debugList:getItemCount()>50)then daa.debugList:removeItem(1)end
...
if(d_a.mouseHandler(dbc,_cc,acc,bcc,ccc))then
...
(cdc:mouseHandler(_cc,acc,bcc,ccc))then return true end end end end end end
if(b_b)then
...
function(ab)local bb,cb=pcall(da,ab)return bb and cb or nil end end;aa["objects"]={}
aa["objects"]["Graphic"]=function(...)local ab=da("Object")
local bb=da("tHex")local cb=da("utils").getValueFromXML;local db=da("bimg")
local _c=da("images")local ac,bc,cc,dc=string.sub,string.len,math.max,math.min
return
function(_d)local ad=ab(_d)
local bd="Graphic"local cd=db()local dd=cd.getFrameObject(1)local __a;local a_a=1
ad:setZIndex(5)local b_a,c_a=0,0
local d_a={getType=function(_aa)return bd end,setOffset=function(_aa,aaa,baa,caa)if(caa)then b_a=b_a+aaa or 0
c_a=c_a+baa or 0 else b_a=aaa or b_a;c_a=baa or c_a end
_aa:updateDraw()return _aa end,getOffset=function(_aa)return
...
cd,dd=_aa,aaa;d_a:updateDraw()return d_a end,getDropdownSize=function(d_a)return cd,dd end,mouseHandler=function(d_a,_aa,aaa,baa,caa)
if
(b_a)then
local daa,_ba=d_a:getAbsolutePosition(d_a:getAnchorPosition())
if(_aa==1)then
if(#bc>0)then
for n=1,dd do
if(bc[n+bd]~=nil)then
if(daa<=aaa)and(daa+cd>aaa)and
(_ba+n==baa)then d_a:setValue(bc[n+bd])
d_a:updateDraw()
local aba=d_a:getEventSystem():sendEvent("mouse_click",d_a,"mouse_click",dir,aaa,baa)if(caa)then d_a:mouseUpHandler(_aa,aaa,baa)end;if(aba==
false)then return aba end;return true end end end end end end
...
ad.eventHandler(ada,bda,cda,dda,__b,a_b)
if(bda=="paste")then
...
a_a-cd,ada.fgColor)cca(ada)ada:updateDraw()end end end,setSelectionColor=function(ada,bda,cda)aba=
...
local function ab(cb)local db={}
db.___value=nil;db.___name=cb;db.___children={}db.___props={}
function db:value()return self.___value end;function db:setValue(_c)self.___value=_c end
function db:name()return self.___name end;function db:setName(_c)self.___name=_c end
function db:children()return self.___children end;function db:numChildren()return#self.___children end
